...
public class EmpController {
	...
	//스프링 MVC의 DispatherServlet이 넘겨주는 VO객체를 서비스단의 메소드를 호출하면서 넘겨준다.
...
//	}
	...
	public ModelAndView login(EmpVO user, HttpServletRequest request) {
		ModelAndView mav = new ModelAndView();
		EmpVO loginOKUser = service.login(user);
		String viewName="";
		if(loginOKUser != null) {
			//로그인 성공
			...
			//새로운 요청을 할때 쿠키로 저장된 세션 id를 request에 저장하여 요청하며 세버에서는 request의 세션 id와 동일한 값이
			...
			viewName = "redirect:/emp/loginPage.do";
		}
